...
import { execute as executorExecute } from "../src/index" 
import {
  ...
  const benchs = await Promise.all(
    Object.entries(benchmarks).map(
      async ([bench, { query, schema, variables }]) => {
        ...
        const graphqlExecutorResult = await executorExecute({
          schema,
          document: query,
        ...
        if (
          JSON.stringify(graphqlExecutorResult) !== JSON.stringify(graphqlJsResult)
        ) {
          // tslint:disable-next-line:no-console
          console.error(
            "graphql-executor",
        ...
        suite.add("graphql-executor", {
          minSamples: 150,
          defer: true,
          fn(deferred: any) {
            const result = executorExecute({
              schema,
              document: query,
              variableValues: variables || {},
            });
