...
export class RepeaterOverflowError extends Error {
  constructor(message: string) {
    ...
    Object.setPrototypeOf(this, this.constructor.prototype); // istanbul ignore next (See: 'https://github.com/graphql/graphql-js/issues/2317')
...
interface PushOperation<T, TNext> {
  ...
  value: Promise<T | undefined>; // The resolve function of the promise return from push.
...
interface NextOperation<T, TReturn, TNext> {
  ...
  value: PromiseLike<TNext> | TNext | undefined; // The resolve function of the promise returned from next.
...
interface RepeaterRecord<T, TReturn, TNext> {
  ...
  state: number; // The function passed to the repeater constructor.

  executor: RepeaterExecutor<T, TReturn, TNext>; // The buffer passed to the repeater constructor.

  buffer: RepeaterBuffer | undefined; // A queue of values which were pushed.

  pushes: Array<PushOperation<T, TNext>>; // A queue of requests for values.

  nexts: Array<NextOperation<T, TReturn, TNext>>; // NOTE: both the push queue and the next queue will never contain values at the same time.
  // A promise which is continuously reassigned and chained so that all repeater iterations settle in order.

  pending: Promise<unknown> | undefined; // The return value of the executor.

  execution: Promise<TReturn | undefined> | undefined; // An error passed to the stop function.

  err: unknown; // A callback set to the resolve function of the promise returned from push.

  onnext: (value?: PromiseLike<TNext> | TNext) => unknown; // A callback set to the resolve function of the stop promise.
