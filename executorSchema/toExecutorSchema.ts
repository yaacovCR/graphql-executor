...
import { getPossibleInputTypes } from './getPossibleInputTypes.ts';
import { TypeTree } from './typeTree.ts';
...
function _toExecutorSchema(schema: GraphQLSchema): ExecutorSchema {
  ...
  function getPossibleTypes(
    ...
    // TODO: add test
    return (
      possibleTypesMap.get(abstractType) ??
      /* c8 ignore next */
      []
    );
  }
  ...
  function isSubType(
    ...
    return (
      subTypesMap.get(abstractType)?.has(maybeSubType) ?? // TODO: add test
      /* c8 ignore start */
      false
      /* c8 ignore stop */
    );
